...
func main() {
	rootCmd, rootVc := cli.NewCommand(nil, nil, "bridge", "BTP Relay CLI")
	cfg := &Config{}
	...
	rootCmd.AddCommand(&cobra.Command{
		Use:   "version",
		Short: "Print bridge version",
		Args:  cobra.NoArgs,
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Println("bridge version", version, build)
		},
	})
	...
	startCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			l := setLogger(cfg, modLevels)
			l.Debugln(cfg.FilePath, cfg.BaseDir)
			if cfg.BaseDir == "" {
				cfg.BaseDir = path.Join(".", ".bridge", cfg.Src.Address.NetworkAddress())
			}
			...
			c, err := NewBridge(&cfg.Config, cfg.KeyStoreData, pw, l)
			if err != nil {
				return err
			}
			return c.Serve()
...
func setLogger(cfg *Config, modLevels map[string]string) log.Logger {
	l := log.GlobalLogger()
