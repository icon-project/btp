...
func (s *sender) MonitorLoop(height int64, callBack MonitorCallback, scb func()) error {
	...
	return s.client.MonitorSenderBlock(blockRequest,
		func(observable rxgo.Observable) error {
			...
			for monitorBlockData := range monitorBlocks {
				...
				blockNotification, _ := monitorBlockData.V.(BlockNotification)
				callBack(blockNotification.Height())
			}
			return nil
		},
		func() {
...
func (s *sender) GetResult(getResultParam GetResultParam) (TransactionResult, error) {
	for {
...
func (s *sender) UpdateSegment(blockProof *BlockProof, segment *Segment) error {
