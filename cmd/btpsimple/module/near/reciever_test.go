...
func TestReceiver_ReceiveLoop(t *testing.T) {
	...
	t.Run("should monitor from the given height", func(t *testing.T) {
		...
		endpoint := ""
		source := base.BtpAddress("btp://0x1.near/53b3d03550bdd3367a686f4f19333e98b61ae6ebb259d906696617305c7bcf7a")
		destination := base.BtpAddress("btp://0x58eb1c.icon/cx9c072bef545abeb96a3f7ba2ffdf8b861fb55dea")

		receiver := base.NewReceiver(source, destination, endpoint, nil, logger, nearClient)
		err = receiver.ReceiveLoop(377826, 0, func(blockUpdate *base.BlockUpdate, receiptProof []*base.ReceiptProof) {
	...
	t.Run("should build receipt proof for the events", func(t *testing.T) {
		...
		err = receiver.ReceiveLoop(377826, 0, func(blockUpdate *base.BlockUpdate, receiptProof []*base.ReceiptProof) {
			...
			if blockUpdate.Height == 377826 {
				...
				assert.EqualValues(t, receiptProof[0].Index, 377826)
