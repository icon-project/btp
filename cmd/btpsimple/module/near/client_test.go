...
import (
	...
	"github.com/icon-project/btp/cmd/btpsimple/module/near/testdata"
		...
		t.Run(test.Description(), func(f *testing.T) {
			for _, testData := range test.TestDatas() {
				f.Run(testData.Description, func(f *testing.T) {
					...
					actual, err := client.GetBlockHeader(int64(input))
					if testData.Expected.Success != nil {
						expected, Ok := (testData.Expected.Success).([]byte)
						assert.True(f, Ok)
		...
		t.Run(test.Description(), func(f *testing.T) {
			for _, testData := range test.TestDatas() {
				f.Run(testData.Description, func(f *testing.T) {
					...
					input, Ok := (testData.Input).([]base.BtpAddress)
					assert.True(f, Ok)
		...
		t.Run(test.Description(), func(f *testing.T) {
			for _, testData := range test.TestDatas() {
				f.Run(testData.Description, func(f *testing.T) {
					...
					input, Ok := (testData.Input).([]string)
					assert.True(f, Ok)
					source := base.BtpAddress(input[0])
					destination := base.BtpAddress(input[1])
		...
		t.Run(test.Description(), func(f *testing.T) {
			...
			for _, testData := range test.TestDatas() {
				...
				f.Run(testData.Description, func(f *testing.T) {
					...
					assert.True(f, Ok)
					transactionParam := base.TransactionParam(&input.TransactionParameters)
		...
		t.Run(test.Description(), func(f *testing.T) {
			...
			for _, testData := range test.TestDatas() {
				...
				f.Run(testData.Description, func(f *testing.T) {
					...
					input, Ok := (testData.Input).(base.TransactionParam)
					assert.True(f, Ok)
		...
		t.Run(test.Description(), func(f *testing.T) {
			...
			for _, testData := range test.TestDatas() {
				...
				f.Run(testData.Description, func(f *testing.T) {
					...
					getResultParam := base.GetResultParam(&blockHashParam)
