...
func Test_Monitor(t *testing.T) {
	t.Run("Test MonitorLoop", func(f *testing.T) {
		...
		logger := log.New()
		endpoint := ""
		source := base.BtpAddress("btp://0x1.icon/0xc294b1A62E82d3f135A8F9b2f9cAEAA23fbD6Cf5")
		destination := base.BtpAddress("btp://0x1.near/dev-20211206025826-24100687319598")
	...
	t.Run("Test_Sender", func(f *testing.T) {
		...
		var blockUpdates []*base.BlockUpdate

		for _, blockupdate := range relaymessage.BlockUpdates {

			var newBlockUpdate base.BlockUpdate
		...
		var reciptProofs []*base.ReceiptProof
		...
		for _, receiptProof := range relaymessage.ReceiptProofs {
			...
			receiptProofs := base.ReceiptProof{
				Index: rP.Index,
				Proof: rP.Proof,
			}
