   ...
   */
    ...
    *****************************************************************************************/
    ...
    function decodeMsgAndValidateRelay(
        ...
        bytes[] memory serializedMsgs = IBMV(bmvServices[_net])
        .handleRelayMessage(bmcAddress, _prev, links[_prev].rxSeq, _msg);

        // rotate and check valid relay
        (uint256 _height, uint256 _lastHeight, ) = IBMV(bmvServices[_net])
        .getStatus();
        address relay = rotateRelay(
            _prev,
            block.number,
            _lastHeight,
        ...
        internal
    {
        if (_msg.svc.compareTo("_EVENT")) {
            Types.EventMessage memory _eventMsg = _msg
            .message
            .decodeEventMessage();
    ...
    event debug(string bmc, string to, string svc, uint256 sn, bytes msg);
   ...
   */
    ...
    */
    function getVerifiers() external view returns (Types.Verifier[] memory) {
        Types.Verifier[] memory verifiers = new Types.Verifier[](
            numOfBMVService
        );
        uint256 temp = 0;
        ...
        */
        ...
        if (_rotateTerm > 0) {
            if (_hasMsg) {
                ...
                */
                uint256 _skipCount = ceilDiv(
                    (_currentHeight - _guessHeight),
                    link.delayLimit
                );
   ...
   */
    ...
    */
        ...
        )
    {
        require(links[_link].isConnected == true, "BMCRevertNotExistsLink");
        Types.Link memory link = links[_link];
        Types.RelayStats[] memory _relays = new Types.RelayStats[](
            link.relays.length
        );
        ...
        uint256 _rotateTerm = getRotateTerm(
            link.maxAggregation,
