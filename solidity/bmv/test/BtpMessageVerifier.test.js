...
const SRC_NETWORK_ID = '0x6274703a2f2f3078312e69636f6e';
const NTID = new BN('2');
const NID = new BN('2');
...
contract('BtpMessageVerifier', (accounts) => {
    ...
    describe('when has installed: BlockUpdate', () => {
        ...
        beforeEach(async () => {
            ...
            await this.instance.initialize(BMC, SRC_NETWORK_ID, NETWORK_TYPE_ID, NETWORK_ID, FIRST_BLOCK_UPDATE);
        });
        ...
        describe('when send RELAY_MESSAGE = [BlockUpdate, MessageProof]', () => {
            ...
            it('returns message', async () => {
                let msgs = await this.instance.handleRelayMessage.call('', '', 0, RELAY_MESSAGE);
                expect(msgs.map(v => toStr(v))).to.deep.equal(['alice']);
            });

            describe('after changed state', () => {
                beforeEach(async () => {
                    await this.instance.handleRelayMessage('', '', 0, RELAY_MESSAGE);
                });
        ...
        describe('when send RELAY_MESSAGES = [[BlockUpdate, MessageProof], [MessageProof]]', () => {
            ...
            it('returns messages: RELAY_MESSAGES[0]', async () => {
                let msgs = await this.instance.handleRelayMessage.call('', '', 0, RELAY_MESSAGES[0]);
                expect(msgs.map(v => toStr(v))).to.deep.equal(['alice']);
            });

            describe('after changed state: RELAY_MESSAGES[0]', () => {
                beforeEach(async () => {
                    await this.instance.handleRelayMessage('', '', 0, RELAY_MESSAGES[0]);
                });
        ...
        describe('when send RELAY_MESSAGES = [[BlockUpdate, MessageProof], [MessageProof], [MessageProof]]', () => {
            ...
            describe('after changed state', () => {
                ...
                describe('when send RELAY_MESSAGES[1]', () => {
                    it('returns messages', async () => {
                        let msgs = await this.instance.handleRelayMessage.call('', '', 0, RELAY_MESSAGES[1]);
                        expect(msgs.map(v => toStr(v))).to.deep.equal(['bob']);
                    });

                    describe('after changed state', () => {
                        beforeEach(async () => {
                            await this.instance.handleRelayMessage('', '', 0, RELAY_MESSAGES[1]);
                        });
                        ...
                        describe('when send RELAY_MESSAGE[2]', () => {
                            it('returns messages', async () => {
                                let msgs = await this.instance.handleRelayMessage.call('', '', 0, RELAY_MESSAGES[2]);
                                expect(msgs.map(v => toStr(v))).to.deep.equal(['crystal']);
                            });

                            describe('after changed state', () => {
                                beforeEach(async () => {
                                    await this.instance.handleRelayMessage('', '', 0, RELAY_MESSAGES[2]);
                                });
        ...
        describe('when send RELAY_MESSAGE for another network', () => {
            ...
            it('revert', async () => {
                await expectRevert(this.instance.handleRelayMessage.call(
                    '', '', 0, RELAY_MESSAGE), "BtpMessageVerifier: BlockUpdate for unknown network",
                );
            });
        });
        ...
        describe('when send RELAY_MESSAGE with invalid signatures', () => {
            ...
            describe('has lack of quorum', () => {
                ...
                it('reverts', async () => {
                    await expectRevert(
                        this.instance.handleRelayMessage.call('', '', 0, RELAY_MESSAGE),
    ...
    describe('when has installed with block update which has generated with message', () => {
        ...
        describe('when miss RELAY_MESSAGE = [MessageProof], and send RELAY_MESSAGE = [BlockUpdate]', () => {
            ...
            it('revert', async () => {
                await expectRevert(
                    this.instance.handleRelayMessage.call('', '', 0, INVALID_RELAY_MESSAGE),
        ...
        describe('when send RELAY_MESSAGE = [MessageProof]', () => {
            const MESSAGE_PROOF = toB64U('0xd0cfce028ccbf800c685616c696365f800');

            it('returns messages', async () => {
                let msgs = await this.instance.handleRelayMessage.call('', '', 0, MESSAGE_PROOF);
                expect(msgs.map(v => toStr(v))).to.deep.equal(['alice']);
            });

            describe('after changed state', () => {
                beforeEach(async () => {
                    await this.instance.handleRelayMessage('', '', 0, MESSAGE_PROOF);
                });
