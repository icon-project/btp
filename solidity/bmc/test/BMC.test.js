...
contract('BMC Basic Unit Tests', (accounts) => {
    ...
    ***************************************************************************************/
    ...
    it('Remove Service - Without Permission - Failure', async () => {
        let service = 'Coin/WrappedCoin';
        await truffleAssert.reverts(
            bmcManagement.removeService(service, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Remove Service - Service Not Existed - Failure', async () => {
        let service = 'Token A';
        await truffleAssert.reverts(
            bmcManagement.removeService(service),
    ...
    ***************************************************************************************/
    ...
    it('Add Verifier - Without Permission - Failure', async () => {
        let network = '1234.iconee';
        await truffleAssert.reverts(
            bmcManagement.addVerifier(network, bmv1.address, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Add Verifier - Service Existed - Failure', async () => {
        let network = '1234.iconee';
        await truffleAssert.reverts(
            bmcManagement.addVerifier(network, bmv1.address),
    ...
    it('Remove Verifier - Without Permission - Failure', async () => {
        let network = '1234.iconee';
        await truffleAssert.reverts(
            bmcManagement.removeVerifier(network, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Remove Verifier - Service Not Existed - Failure', async () => {
        ...
        await truffleAssert.reverts(
            bmcManagement.removeVerifier(network),
    ...
    ***************************************************************************************/
    ...
    it('Add Link - Without Permission - Failure', async () => {
        let link = 'btp://1234.pra/0x1234';
        await truffleAssert.reverts(
            bmcManagement.addLink(link, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });  
    ...
    it('Add Link - Invalid BTP Address Format - Failure', async () => {
        ...
        assert(res, 'Error: Returned error: VM Exception while processing transaction: invalid opcode');
    });  
    ...
    it('Remove Link - Without Permission - Failure', async () => {
        let link = 'btp://1234.pra/0x1234';
        await truffleAssert.reverts(
            bmcManagement.removeLink(link, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    ***************************************************************************************/
    it('Set Link - Without Permission - Failure', async () => {
        ...
        let delayLimit = 4;
        await truffleAssert.reverts(
            bmcManagement.setLink(link, blockInterval, maxAggregation, delayLimit, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Set Link - Link not Existed - Failure', async () => {
        ...
        let delayLimit = 3;
        await truffleAssert.reverts(
            bmcManagement.setLink(link, blockInterval, maxAggregation, delayLimit),
            "BMCRevertNotExistsLink"
        );
    });
    ...
    ***************************************************************************************/
    ...
    it('Add Route - Without Permission - Failure', async () => {
        ...
        let link = 'btp://1234.pra/0x1234';
        await truffleAssert.reverts(
            bmcManagement.addRoute(dst, link, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Add Route - Route Existed - Failure', async () => {
        ...
        let link = 'btp://1234.pra/0x1234';
        await truffleAssert.reverts(
            bmcManagement.addRoute(dst, link),
    ...
    it('Remove Route - Without Permission - Failure', async () => {
        let dst = 'btp://1234.iconee/0x1234';
        await truffleAssert.reverts(
            bmcManagement.removeRoute(dst, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    ***************************************************************************************/
    ...
    it('Add Relays - Without Permission - Failure', async () => {
        ...
        await truffleAssert.reverts(
            bmcManagement.addRelay(link, relays, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Add Relays - Link not existed - Failure', async () => {
        ...
        let relays = [accounts[2], accounts[3], accounts[4]];
        await truffleAssert.reverts(
            bmcManagement.addRelay(link, relays),
            "BMCRevertNotExistsLink"        
        );
    });
    ...
    it('Remove Relay - Without Permission - Failure', async () => {
        ...
        await truffleAssert.reverts(
            bmcManagement.removeRelay(link, relay, {from: accounts[1]}),
            "BMCRevertUnauthorized"
        );
    });
    ...
    it('Remove Relay - Link Not Existed - Failure', async () => {
        ...
        let relay = accounts[2];
        await truffleAssert.reverts(
            bmcManagement.removeRelay(link, relay),
            "BMCRevertUnauthorized"
        );
    });
...
contract('BMC Relay Rotation Unit Test - Rotate_term = 0', (accounts) => {
...
contract('Handle relay message', (accounts) => {
    ...
    it('should revert if relay is invalid', async() => {
        ...
        await truffleAssert.reverts(bmcPeriphery.handleRelayMessage(link, relayMsg), 'BMCRevertUnauthorized: invalid relay');
    ...
    it('should revert if relay is not registered', async() => {
        ...
        await truffleAssert.reverts(bmcPeriphery.handleRelayMessage(link, 'base64EncodeRelayMessage'), 'BMCRevertUnauthorized: not registered relay');
...
contract('Test upgradable contracts', (accounts) => {
    ...
    it('should upgrade BMC Periphery', async() => {
        ...
        await truffleAssert.reverts(
            upgradeBMCPeriphery.requestAddService('RandomService', bsh.address),
