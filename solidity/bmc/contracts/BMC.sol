...
contract BMC is IBMC {
    ...
    mapping(string => address) internal bshServices;    // should be private, temporarily set internal for testing
    ...
    string public bmcAddress; // a network address BMV, i.e. btp://1234.pra/0xabcd
   ...
   */
    ...
    *****************************************************************************************/
        ...
        override
    {
        ...
        for (uint256 i = 0; i < serializedMsgs.length; i++) {
            this.decodeBTPMessage(serializedMsgs[i]);
    ...
    function decodeMsgAndValidateRelay(
        ...
        if (relay == address(0)) {
            address[] memory relays = getRelays(_prev);
            bool check;
        ...
        if (_msg.svc.compareTo("_EVENT")) {
            ...
            if (_eventMsg.eventType.compareTo("Link")) {
                ...
                if (links[_eventMsg.conn.from].isConnected) {
                    for (
                        uint256 i = 0;
                        i < links[_eventMsg.conn.from].reachable.length;
                        i++
                    )
                        if (
                            _eventMsg.conn.to.compareTo(
                                links[_eventMsg.conn.from].reachable[i]
                            )
                        ) {
                            check = true;
                            break;
                    ...
                    if (!check)
                        links[_eventMsg.conn.from].reachable.push(_eventMsg.conn.to);
                }
                ...
                if (links[_eventMsg.conn.from].isConnected) {
                    for (
                        ...
                        ) delete links[_eventMsg.conn.from].reachable[i];
                    }
                }
            ...
            if (_msg.sn >= 0) {
                (string memory _net, ) = _msg.dst.splitBTPAddress();
                try
        ...
        returns (string memory)
    {
        ...
        if (bytes(routes[_dst]).length == 0) {
            for (uint256 i = 0; i < listLinkNames.length; i++) {
                if (listLinkNames[i].compareTo(_dst)) return listLinkNames[i];

                for (
                    uint256 j = 0;
                    j < links[listLinkNames[i]].reachable.length;
                    j++
                )
                    if (links[listLinkNames[i]].reachable[j].compareTo(_dst))
                        return listLinkNames[i];
            }
    ...
    */
    function sendMessage(
        ...
        bytes memory _rlp =
            Types
                .BMCMessage(bmcAddress, _to, _svc, int256(_sn), _msg)
                .encodeBMCMessage();
        ...
        if (_rotateTerm > 0) {
            if (_hasMsg) {
                ...
                uint256 _guessHeight =
                    link.rxHeight +
                        ceilDiv(
                            (_relayMsgHeight - link.rxHeightSrc) * 10**6,
                            _scale
                        ) -
                        1;
