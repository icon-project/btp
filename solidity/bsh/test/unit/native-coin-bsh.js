// const MockBSHService = artifacts.require("MockBSH");
// const BSHService = artifacts.require("BSHPeriphery");
// const BSHCoin = artifacts.require("BSHCore");
...
contract.skip('BSHService Basic Unit Tests', () => {
    let bsh_svc, bsh_coin, accounts;
    var native_coin_name = 'PARA';  var fee = 10;
    var service = 'Coin/WrappedCoin';  

    before(async () => {
        bmc = await BMC.deployed();
        bsh_svc = await BSHPeriphery.deployed();
        bsh_coin = await BSHCore.deployed();
        // await bsh_svc.initialize(bmc.address, bsh_coin.address, service, native_coin_name, fee);
        // accounts = await web3.eth.getAccounts();
    });

    it('Contracts code size: ', async() => {
        console.log('BSHService code size: ', BSHPeriphery.deployedBytecode.length / 2);
        console.log('BSHCoin code size: ', BSHCore.deployedBytecode.length / 2);
    });

    it('Register Coin - With Permission - Success', async () => {
        var _name = "ICON";
        await bsh_svc.register(_name);
        output = await bsh_svc.coinNames();
        assert(
    ...
    it('Register Coin - Without Permission - Failure', async () => {   
        ...
        await truffleAssert.reverts(
            bsh_svc.register(_name, {from: accounts[1]}),
    ...
    it('Coin Query - Valid Supported Coin - Success', async () => {
        ...
        await bsh_svc.register(_name1);
        await bsh_svc.register(_name2);

        var _query = "ICON";
        var id = await bmc.hashCoinName(_query);
        var result = await bsh_svc.coinOf(_query);
        assert(
    ...
    it('Contract Owner Transfer - Success', async () => {
        var oldOwner = await bsh_svc.owner();
        await bsh_svc.transferOwnership(accounts[1]);
        var newOwner = await bsh_svc.owner();
        assert(
    ...
    it('Register Coin - New Owner - Success', async () => {   
        var _name3 = "TRON";
        await bsh_svc.register(_name3, {from: accounts[1]});
        output = await bsh_svc.coinNames();
        assert(
            output[0] === native_coin_name && output[1] === 'ICON' &&
            output[2] === 'wBTC' && output[3] === 'Ethereum' &&
...
contract.skip('BSHService Transfer Coin Unit Tests', () => {
    let bsh_svc, bsh_coin, bmc, nonrefundable, refundable, accounts;
    var service = 'Coin/WrappedCoin';               
    ...
    before(async () => {
        bsh_svc = await MockBSHService.deployed();
        bsh_coin = await BSHCoin.deployed();
        bmc = await BMC.deployed();
        await bsh_svc.initialize(bmc.address, bsh_coin.address, service, _native, fee);
        await bsh_coin.initialize();
        await bsh_coin.setBSHService(bsh_svc.address);
        nonrefundable = await NonRefundable.deployed();
        refundable = await Refundable.deployed();
        accounts = await web3.eth.getAccounts();
        await bmc.setBSH(bsh_svc.address);
    ...
    it('Transfer Native Coin to side chain - From Account - Success', async () => {
        var balanceBefore = await bsh_coin.getBalanceOf(accounts[0], _native);
        var output = await bsh_svc.transfer(_to, {from: accounts[0], value: _amt1});
        var balanceAfter = await bsh_coin.getBalanceOf(accounts[0], _native);
    ...
    it('Transfer Native Coin to side chain - From Account - Success', async () => {
        var balanceBefore = await bsh_coin.getBalanceOf(accounts[0], _native);
        var output = await bsh_svc.transfer(_to, {from: accounts[0], value: _amt2});
        var balanceAfter = await bsh_coin.getBalanceOf(accounts[0], _native);
    ...
    it('Transfer Native Coin to side chain - From Non-Refundable Contract - Success', async () => {
        ...
        var balanceBefore = await bsh_coin.getBalanceOf(nonrefundable.address, _native);
        var _usableBalanceBefore = await bsh_svc.getBalance(nonrefundable.address);
        await nonrefundable.transfer(bsh_svc.address, _to, _amt3);
        var balanceAfter = await bsh_coin.getBalanceOf(nonrefundable.address, _native);
        var _usableBalanceAfter = await bsh_svc.getBalance(nonrefundable.address);
    ...
    it('Transfer Native Coin to side chain - From Refundable Contract - Success', async () => {
        ...
        var _usableBalanceBefore = await bsh_svc.getBalance(refundable.address);
        var balanceBefore = await bsh_coin.getBalanceOf(refundable.address, _native);
        await refundable.transfer(bsh_svc.address, _to, _amt3);
        var balanceAfter = await bsh_coin.getBalanceOf(refundable.address, _native);
        var _usableBalanceAfter = await bsh_svc.getBalance(refundable.address);
    ...
    it('Transfer Native Coin to side chain - Invalid BTP Address format - Failure', async () => {
        ...
        var rsp = '';
        try {
            await bsh_svc.transfer(invalid_destination, {from: accounts[0], value: 1000});
        }catch (error) {
            rsp = error.toString();
        }
        assert(rsp == err);
    ...
    it('Transfer Native Coin to side chain - Amount is Zero - Failure' , async () => {
        await truffleAssert.reverts(
            bsh_svc.transfer(_to, {from: accounts[0], value: 0}),
            "InvalidAmt"
        ); 
    });
    ...
    it('Transfer Native Coin to side chain - Charge Fee is Zero - Failure' , async () => {
        await truffleAssert.reverts(
            bsh_svc.transfer(_to, {from: accounts[0], value: 10}),
            "InvalidAmt"
        ); 
    });
    ...
    it('Transfer Native Coin to side chain - Invalid Network/Network Address Not Supported - Failure' , async () => {
        ...
        await truffleAssert.reverts(
            bsh_svc.transfer(invalid_destination, {from: accounts[1], value: 1000000}),
            "BMCRevertNotExistsBMV"
        ); 
    });
});

contract.skip('BSHService Transfer Token Unit Tests', () => {
    let bsh_svc, bsh_coin, bmc, holder, accounts;
    var service = 'Coin/WrappedCoin';
    var native_coin_name = 'PARA';  var fee = 10;
    var _name = 'ICON';
    ...
    before(async () => {
        bmc = await BMC.deployed();
        bsh_svc = await BSHService.deployed();
        bsh_coin = await BSHCoin.deployed();
        ...
        await bmc.approveService(service);
        await bmc.addVerifier(_net, accounts[1]);
        await holder.addBSHContract(bsh_svc.address, bsh_coin.address);
        await bsh_svc.register(_name);
        await bmc.transferRequestWithAddress(
            _net, service, _from, holder.address, _name, _value
        );
        id = await bsh_svc.coinOf(_name);
    });

    it('Transfer Token to side chain - Not set Approval - Failure', async () => {
        var _to = 'btp://1234.iconee/0x12345678';
        var _value = 1000;
        var balanceBefore = await bsh_coin.getBalanceOf(holder.address, _name);
        var _usableBalanceBefore = await bsh_coin.balanceOf(holder.address, id);
        await truffleAssert.reverts(
            holder.callTransfer(_name, _value, _to),
            "ERC1155: caller is not owner nor approved"
        ); 
        var balanceAfter = await bsh_coin.getBalanceOf(holder.address, _name);
        var _usableBalanceAfter = await bsh_coin.balanceOf(holder.address, id);
        assert(
            web3.utils.BN(_usableBalanceAfter).toNumber() == 
                web3.utils.BN(_usableBalanceBefore).toNumber() &&
            web3.utils.BN(balanceBefore._lockedBalance).toNumber() == 0 &&    
    ...
    it('Transfer Token to side chain - Insufficient Balance - Failure', async () => {
        ...
        var balanceBefore = await bsh_coin.getBalanceOf(holder.address, _name);
        var _usableBalanceBefore = await bsh_coin.balanceOf(holder.address, id);
        await holder.setApprove(bsh_coin.address);
        await truffleAssert.reverts(
            holder.callTransfer(_name, _value, _to),
...
contract.skip('BSHService Handles BTP Message - Receive PARA Coin Mint Request - Unit Tests', () => {
    let bmc, bsh_svc, bsh_coin, accounts, notpayable, refundable;
    var service = 'Coin/WrappedCoin';
    var _net = '1234.iconee';   var _to = 'btp://1234.iconee/0x12345678';
    var _native = 'PARA';   var fee = 10;
    ...
    before(async () => {
        ...
        await bmc.approveService(service);
        await bmc.addVerifier(_net, accounts[1]);
        await bsh_svc.transfer(_to, {from: accounts[0], value: 100000000});
    });
...
contract.skip('BSHService Handles BTP Message - Receive Token Mint Request - Unit Tests', () => {
    let bmc, bsh_svc, bsh_coin, holder, notpayable, accounts;
    var service = 'Coin/WrappedCoin';
    var native_coin_name = 'PARA';  var fee = 10;
    var _name = 'ICON';
    ...
    it('Receive Request Token Mint - Invalid address - Failure', async () => {
        var _value = 1000;
        var _address = '0x1234567890123456789';
        await truffleAssert.reverts( 
            bmc.transferRequestWithStringAddress(
    ...
    it('Receive Request Token Mint - Receiver not ERC1155Holder/Receiver - Failure', async () => {
        var _value = 1000;
        var balanceBefore = await bsh_coin.balanceOf(notpayable.address, id);
        await truffleAssert.reverts( 
            bmc.transferRequestWithAddress(
                _net, service, _from, notpayable.address, _name, _value),
            'MintFailed'    
        );
        var balanceAfter = await bsh_coin.balanceOf(notpayable.address, id);
        assert(
            web3.utils.BN(balanceAfter).toNumber() == web3.utils.BN(balanceBefore).toNumber()
        );
    });
    ...
    it('Receive Request Token Mint - Invalid Token Name - Failure', async () => {
        ...
        var invalid_coin_id = await bsh_svc.coinOf(_tokenName);
        var balanceBefore = await bsh_coin.balanceOf(holder.address, invalid_coin_id);
        await truffleAssert.reverts(
            bmc.transferRequestWithAddress(
            _net, service, _from, holder.address, _tokenName, _value),
            'InvalidToken'
        );
        var balanceAfter = await bsh_coin.balanceOf(holder.address, invalid_coin_id);
        assert(
            web3.utils.BN(balanceAfter).toNumber() == web3.utils.BN(balanceBefore).toNumber()
        );
    });
    ...
    it('Receive Request Token Mint - ERC1155Holder/Receiver Contract - Success', async () => { 
        var _value = 2500;
        var balanceBefore = await bsh_coin.balanceOf(holder.address, id);
        await bmc.transferRequestWithAddress(
            _net, service, _from, holder.address, _name, _value
        );
        var balanceAfter = await bsh_coin.balanceOf(holder.address, id);
        assert(
    ...
    it('Receive Request Token Mint - Account - Success', async () => { 
        var _value = 5500;
        var balanceBefore = await bsh_coin.balanceOf(accounts[1], id);
        await bmc.transferRequestWithAddress(
            _net, service, _from, accounts[1], _name, _value
        );
        var balanceAfter = await bsh_coin.balanceOf(accounts[1], id);
        assert(
...
contract.skip('BSHService Handle Fee Aggregation', () => {
    let bsh_svc, bsh_coin, bmc, holder, accounts;
    var service = 'Coin/WrappedCoin';
    var _native = 'PARA';       var fee = 10;
    ...
    before(async () => {
        ...
        await bsh_svc.register(_name1);
        await bsh_svc.register(_name2);
        await bsh_svc.register(_name3);
        await bsh_svc.register(_name4);
        await bmc.transferRequestWithAddress(
        ...
        await bsh_svc.transfer(_to1, {from: accounts[0], value: _txAmt});
    ...
    it('Query Aggregation Fee', async () => {
        var aggregationFee = await bsh_coin.getAccumulatedFees();
        assert(
    ...
    it('BSHService Handle Gather Fee Request - Not from BMC - Failure', async () => {
        var FA1Before = await bsh_svc.getAggregationFeeOf(_native);     //  state Aggregation Fee of each type of Coins
        var FA2Before = await bsh_svc.getAggregationFeeOf(_name1);
        var FA3Before = await bsh_svc.getAggregationFeeOf(_name2);
        await truffleAssert.reverts( 
            bsh_svc.handleFeeGathering(_to1, service, {from: accounts[1]}),
            'Unauthorized'    
        );
        var FA1After = await bsh_svc.getAggregationFeeOf(_native);
        var FA2After = await bsh_svc.getAggregationFeeOf(_name1);
        var FA3After = await bsh_svc.getAggregationFeeOf(_name2);
        var fees = await bsh_svc.getFees(_sn2 + 1);     //  get pending Aggregation Fee list
        assert(
    ...
    it('BSHService Handle Gather Fee Successful Response', async () => {
        var feesBefore = await bsh_svc.getFees(_sn2 + 1);
        await bmc.response(REPONSE_HANDLE_SERVICE, _net1, service, _sn2+1, RC_OK, "");
        var feesAfter = await bsh_svc.getFees(_sn2 + 1);
        assert(
            feesBefore.length == 3 &&
    ...
    it('BSHService Handle Gather Fee Error Response', async () => {
        ...
        var FA1Before = await bsh_svc.getAggregationFeeOf(_name1);
        var FA2Before = await bsh_svc.getAggregationFeeOf(_name2);
        var feesBefore = await bsh_svc.getFees(_sn2 + 4);
        await bmc.response(REPONSE_HANDLE_SERVICE, _net1, service, _sn2+4, RC_ERR, "");
        var FA1After = await bsh_svc.getAggregationFeeOf(_name1);
        var FA2After = await bsh_svc.getAggregationFeeOf(_name2);
        var feesAfter = await bsh_svc.getFees(_sn2 + 4);
        assert(
            feesBefore.length == 2 &&
...
contract.skip('BSHCoin Basic Unit Tests', () => {
    let bmc, bsh_svc, bsh_coin, notpayable, accounts;
    var service = 'Coin/WrappedCoin';
    var native_coin_name = 'PARA';  var fee = 10;
    ...
    it('BSHCoin receives Mint Token Request - From BSHService Contract - Success', async () => { 
        var _value = 2500;
        var balanceBefore = await bsh_coin.balanceOf(holder.address, id);
        await bsh_svc.mint(holder.address, id, _value);
        var balanceAfter = await bsh_coin.balanceOf(holder.address, id);
        assert(
            web3.utils.BN(balanceAfter).toNumber() == web3.utils.BN(balanceBefore).toNumber() + 2500
        );
    ...
    //     var _value = 5500;
    //     var balanceBefore = await bsh_coin.balanceOf(holder.address, id);
    //     await bsh_coin.mint(holder.address, coin.id, _value);
    //     var balanceAfter = await bsh_coin.balanceOf(holder.address, id);
    //     assert(
    //         web3.utils.BN(balanceAfter).toNumber() == web3.utils.BN(balanceBefore).toNumber() + 5500
    //     );
    ...
    it('BSHCoin receives Mint Token Request - Not From BSHService Contract Nor Owner - Failure', async () => { 
        ...
        var balanceBefore = await bsh_coin.balanceOf(holder.address, id);
        await truffleAssert.reverts( 
            bsh_coin.refund(holder.address, id, _value, false, {from: accounts[1]}),
            'Unauthorized'    
        );
        var balanceAfter = await bsh_coin.balanceOf(holder.address, id);
        assert(
    ...
    it('BSHCoin receives Mint Token Request - Mint Token to an Account - Success', async () => { 
        var _value = 5500;
        var balanceBefore = await bsh_coin.balanceOf(accounts[1], id);
        await bsh_svc.mint(accounts[1], id, _value);
        var balanceAfter = await bsh_coin.balanceOf(accounts[1], id);
        assert(
            web3.utils.BN(balanceAfter).toNumber() == web3.utils.BN(balanceBefore).toNumber() + 5500
        );
    ...
    it('BSHCoin receives Mint Token Request - Mint Token to a Non ERC1155Holder/Receiver Contract - Failure', async () => { 
        ...
        var balanceBefore = await bsh_coin.balanceOf(notpayable.address, id);
        await truffleAssert.reverts( 
            bsh_svc.mint(notpayable.address, id, _value),
    ...
    it('BSHCoin receives Burn Token Request - From BSHService Contract - Success', async () => { 
        ...
        await holder.setApprove(bsh_coin.address);
        await holder.callTransfer(_name, _value, _to);
        var balanceBefore = await bsh_coin.balanceOf(bsh_coin.address, id);
        await bsh_svc.burn(id, _value);
        var balanceAfter = await bsh_coin.balanceOf(bsh_coin.address, id);
        assert(
    ...
    //     var _value = 1000;
    //     var balanceBefore = await bsh_coin.balanceOf(holder.address, coin.id);
    //     await bsh_coin.burn(holder.address, coin.id, _value);
    //     var balanceAfter = await bsh_coin.balanceOf(holder.address, coin.id);
    //     assert(
    ...
    it('BSHCoin receives Burn Token Request - Not From BSHService Contract Nor Owner - Failure', async () => { 
        ...
        var balanceBefore = await bsh_coin.balanceOf(bsh_coin.address, id);
        await truffleAssert.reverts( 
            bsh_coin.burn(id, _value, {from: accounts[1]}),
            'Unauthorized'    
        );
        var balanceAfter = await bsh_coin.balanceOf(bsh_coin.address, id);
        assert(
    ...
    it('Contract Owner Transfer - Success', async () => {
        var oldOwner = await bsh_coin.owner();
        await bsh_coin.transferOwnership(accounts[1]);
        var newOwner = await bsh_coin.owner();
        assert(
            oldOwner == accounts[0] && newOwner == accounts[1]
        );
    }); 
    ...
    //     var balanceBefore = await bsh_coin.balanceOf(holder.address, coin.id);
    //     await truffleAssert.reverts( 
    //         bsh_coin.mint(holder.address, coin.id, _value),
    //         'Unauthorized'    
    //     );
    //     var balanceAfter = await bsh_coin.balanceOf(holder.address, coin.id);
    //     assert(
    ...
    //     var _value = 30000;
    //     var balanceBefore = await bsh_coin.balanceOf(holder.address, coin.id);
    //     await bsh_coin.mint(holder.address, coin.id, _value, {from: accounts[1]}); 
    //     var balanceAfter = await bsh_coin.balanceOf(holder.address, coin.id);
    //     assert(
