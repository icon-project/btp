def bridge(name, chains):
    ...
    native.genrule(
        ...
        $(execpath @btp//cmd/btpsimple:btpsimple) --key_password $$(cat $(location @%s//cli:keysecret)) --key_store $$(cat $(location @%s//:bmr_config_dir))/keystore.json \
            ...
            --offset $$(cat $(location @%s//:latest_block_height)) \
        ...
        save $@; cp $@ $$(cat $(location @%s//:bmr_config_dir))""" % (chains[1], chains[0], chains[1], chains[0],chains[0],chains[1], chains[0], chains[0], chains[1], chains[1], chains[0]),
        local = True,
        executable = True,
    )
    ...
    native.genrule(
        ...
        $(execpath @btp//cmd/btpsimple:btpsimple) --key_password $$(cat $(location @%s//cli:keysecret)) --key_store $$(cat $(location @%s//:bmr_config_dir))/keystore.json \
            ...
            --dst.options "mtaRootSize"=8 \
            --src.options "mtaRootSize"=8 \
        save $@; cp $@ $$(cat $(location @%s//:bmr_config_dir))""" % (chains[0], chains[1], chains[0], chains[1],chains[1],chains[0] chains[1], chains[1], chains[0], chains[0], chains[1]),
        local = True,
        executable = True,
    )
