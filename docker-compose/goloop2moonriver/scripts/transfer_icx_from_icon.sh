...
bob_balance_in_moonbeam_before_transfering() {
    cd $CONFIG_DIR
    eth abi:add bshcore abi.bsh_core_erc20.json

    RESULT=$(eth contract:call --network $MOONBEAM_RPC_URL bshcore@$(cat bsh_core_erc20.moonbeam) "getBalanceOf('$(get_bob_address)', 'ICX')")
...
echo "This script demonstrates how to transfer a ICX from ICON to MOONBEAM."
