/**
 * All rights reserved to https://github.com/PureStake/moonbeam/tree/v0.11.3/tools
 *
 ...
 */
 ...
 function start() {
   ...
   for (let i = 0; i < parachainBinaries.length; i++) {
     ...
     parachainConfig.nodes[0].flags.unshift(`--rpc-port=${parachainConfig.nodes[0].rpcPort}`)
     ...
     parachainConfig.nodes[1].flags.unshift(`--rpc-port=${parachainConfig.nodes[1].rpcPort}`)
     ...
     relayNodeConfig[0].flags.push(`--rpc-port=${relayNodeConfig[0].rpcPort}`)
     relayNodeConfig[1].flags.push(`--rpc-port=${relayNodeConfig[1].rpcPort}`)
