...
def configure_bmr(name):
    ...
    native.genrule(
        ...
        cmd = "$(execpath @com_github_icon_project_goloop//cmd/goloop:goloop) rpc --uri $$(cat $(location @icon//:wait_until_icon_up))/api/v3/icon sendtx transfer --to $$(cat $(location @icon//cli:get_wallet_%s_keystore)) --value 1000000000000000000000000 --key_store $$(cat $(location @icon//:goloop_config_dir))/keystore.json --key_secret $$(cat $(location @icon//:goloop_config_dir))/keysecret --nid \"$$(cat $(location @icon//:wait_for_channel_up))\" --step_limit 13610920001 && echo \"$$(cat $(location @icon//cli:generate_%s_keystore))\"| jq -r . >$@" % (name, name),
        executable = True,
        local = True,
    )
