...
public class BTPEvent {
    ...
    private List<byte[]> evmTopics = new ArrayList<byte[]>(3);
    private final String next;
    private final BigInteger seq;
    private final byte[] msg;
    private final List<RlpType> proofs = new ArrayList<RlpType>(5);

    public BTPEvent(String prevBmcBtpAddress, byte[] prevBmcAddress, String next, BigInteger seq, byte[] msg) {
        this.next = next;
        this.seq = seq;
        this.msg = msg;

        if (next.length() > 64 || next.length() < 32) {
            throw new AssertionError("invalid next length");
        }

        ByteSliceOutput evmData = new ByteSliceOutput(100);
        AbiEncoder.encodeUInt(evmData, 96); // next position;
        AbiEncoder.encodeUInt(evmData, seq.longValue());
        AbiEncoder.encodeUInt(evmData, 192);
        AbiEncoder.encodeBytes(evmData, next.getBytes());
        AbiEncoder.encodeBytes(evmData, msg);

        ByteSliceOutput eventData = new ByteSliceOutput(100);
        eventData.addMany(prevBmcAddress);
        ScaleWriter.writeCompactUint(eventData, 1); // topic size;
        eventData.addMany(HexConverter.hexStringToByteArray("37be353f216cf7e33639101fd610c542e6a0c0109173fa1c1d8b04d34edb7c1b")); // btp event topic
        ScaleWriter.writeCompactUint(eventData, evmData.size()); // evm data size
        eventData.addMany(evmData.toArray());
        ...
        eventLeafNode.addMany(HexConverter.hexStringToByteArray("012200")); // evm event phase, index
