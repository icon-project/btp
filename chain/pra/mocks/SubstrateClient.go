...
import (
	pra "github.com/icon-project/btp/chain/pra"
...
// GetReadProof provides a mock function with given fields: key, hash
func (_m *SubstrateClient) GetReadProof(key types.StorageKey, hash types.Hash) (pra.ReadProof, error) {
	ret := _m.Called(key, hash)

	var r0 pra.ReadProof
	if rf, ok := ret.Get(0).(func(types.StorageKey, types.Hash) pra.ReadProof); ok {
		r0 = rf(key, hash)
	} else {
		r0 = ret.Get(0).(pra.ReadProof)
	}
