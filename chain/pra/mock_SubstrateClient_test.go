...
func (_m *MockSubstrateClient) GetReadProof(key types.StorageKey, hash types.Hash) (ReadProof, error) {
	ret := _m.Called(key, hash)

	var r0 ReadProof
	if rf, ok := ret.Get(0).(func(types.StorageKey, types.Hash) ReadProof); ok {
		r0 = rf(key, hash)
	} else {
		r0 = ret.Get(0).(ReadProof)
	}
