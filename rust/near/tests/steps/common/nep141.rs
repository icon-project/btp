...
use test_helper::types::{
    Context, Contract, Nep141, Nep141Contract, Nep141Testable, Nep141TestableContract,
};

lazy_static! {
    pub static ref NEP141_CONTRACT: Contract<'static, Nep141> =
        Nep141Contract::new("nep141service", "res/NEP141_CONTRACT.wasm");
    pub static ref NEP141_CONTRACT_TESTABLE: Contract<'static, Nep141Testable> =
        Nep141TestableContract::new("nep141service", "res/NEP141_CONTRACT.wasm");
}

pub static NEP141_CONTRACT_IS_DEPLOYED: fn(Context) -> Context =
    |context| NEP141_CONTRACT.deploy(context);

pub static NEP141_CONTRACT_TESTABLE_IS_DEPLOYED: fn(Context) -> Context =
    |context| NEP141_CONTRACT_TESTABLE.deploy(context);

pub static NEP141_CONTRACT_IS_INITIALIZED: fn(Context) -> Context = |mut context: Context| {
...
pub static NEP141_CONTRACT_IS_DEPLOYED_AND_INITIALZIED: fn(Context) -> Context =
    |mut context: Context| {
        context
            ...
            .pipe(NEP141_CONTRACT_IS_INITIALIZED)
    };
...
pub static NEP141_CONTRACT_TESTABLE_INITIALIZED: fn(Context) -> Context = |mut context: Context| {
    context.add_method_params(
        "new",
        json!({
    ...
    NEP141_CONTRACT_TESTABLE.initialize(context)
};

pub static NEP141_CONTRACT_TESTABLE_IS_DEPLOYED_AND_INITIALZIED: fn(Context) -> Context = |mut context: Context| {
    context
        .pipe(NEP141_CONTRACT_TESTABLE_IS_DEPLOYED)
        .pipe(NEP141_CONTRACT_TESTABLE_INITIALIZED)
};
