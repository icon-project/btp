...
pub static REQUEST_IN_BMC_ARE_QUERIED: fn(Context) -> Context = |context: Context| BMC_CONTRACT.get_requests(context);
...
pub static SERVICE_NAME_AND_SMARTCONTRACT_ADDRESS_PROVIDED_AS_REQUEST_SERVICE_PARAM: fn(Context) -> Context = |mut context: Context| {
...
pub static SERVICE_NAME_AND_INVALID_SMARTCONTRACT_ADDRESS_PROVIDED_AS_REQUEST_SERVICE_PARAM: fn(Context) -> Context = |mut context: Context| {
    let bob = context.accounts().get("bob").to_owned();
    context.add_method_params(
        "request_service",
        json!({
...
pub static BOB_INVOKES_REQUEST_SERVICE_IN_BMC_FROM_BSH: fn(Context) -> Context = |mut context: Context| {
    let signer = context.accounts().get("bob").to_owned();
    context.set_signer(&signer);
    BMC_CONTRACT.request_service(context)
};

pub static BOB_INVOKES_REQUEST_SERVICE_IN_BMC_FROM_BSH_AGAIN: fn(Context) -> Context = |mut context: Context| {
    let signer = context.accounts().get("bob").to_owned();
    context.set_signer(&signer);
    BMC_CONTRACT.request_service(context)
};
...
pub static CONTRACTS_ARE_DEPLOYED: fn(Context) -> Context = |mut context: Context| {
    BMC_CONTRACT_IS_DEPLOYED(context)
    .pipe(BSH_CONTRACT_IS_DEPLOYED)
};
...
pub static CHUCK_INVOKES_REQUEST_SERVICE_IN_BMC_FROM_BSH: fn(Context) -> Context = |mut context: Context| {
...
pub static BMC_SHOULD_THROW_INVALIDADDRESS_ERROR: fn(Context) -> Context = |context: Context| context;
