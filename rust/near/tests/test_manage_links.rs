    ...
    mod bmc {
        ...
        async fn bmr_operator_can_get_link_status_of_connected_bmc() {
            Kitten::given(NEW_CONTEXT)
                ...
                .and(ICON_LINK_IS_PRESENT_IN_BMC)
                .and(ICON_LINK_ADDRESS_IS_PROVIDED_AS_SET_LINK_PARAM)
                .and(SET_LINK_IN_BMC_IS_INVOKED_BY_ALICE)
                .and(ICON_LINK_ADDRESS_IS_PROVIDED_AS_GET_STATUS_PARAM)
                .when(ALICE_INVOKES_GET_STATUS_IN_BMC)
                .then(ICON_LINK_STATUS_SHOULD_BE_UPDATED);
        }
        ...
        #[tokio::test(flavor = "multi_thread")]
        async fn set_max_aggregation_to_link_status() {
            Kitten::given(NEW_CONTEXT)
                .and(BMC_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
                .and(BMV_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
                .and(BMC_CONTRACT_IS_OWNED_BY_ALICE)
                .and(ICON_LINK_IS_PRESENT_IN_BMC)
                .and(ICON_LINK_ADDRESS_IS_PROVIDED_AS_SET_LINK_PARAM)
                .and(SET_LINK_IN_BMC_IS_INVOKED_ALICE)
        ...
        #[tokio::test(flavor = "multi_thread")]
        async fn set_link_as_unauthorized_success(){
            Kitten::given(NEW_CONTEXT)
            .and(BMC_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
            .and(BMV_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
            .and(BMC_CONTRACT_IS_OWNED_BY_ALICE)
        ...
        #[tokio::test(flavor = "multi_thread")]
        async fn set_link_with_delay_limit_less_than_1_fails(){
            Kitten::given(NEW_CONTEXT)
            .and(BMC_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
            .and(BMV_CONTRACT_IS_DEPLOYED_AND_INITIALIZED)
            .and(BMC_CONTRACT_IS_OWNED_BY_ALICE)
            ...
            .when(ALICE_INVOKES_SET_LINK_IN_BMC)
            .then(BMC_SHOULD_THROW_INVALID_PARAM_ERROR_ON_SET_LINK);
        }
